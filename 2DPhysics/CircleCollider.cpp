...
bool CircleCollider::Collides(std::shared_ptr<Vector3>collider)
{
...
bool CircleCollider::CheckCollision(std::shared_ptr<LineCollider>collider)
{
	if (Collides(collider->p1) || Collides(collider->p2))
	{
		/*float Distance = sqrtf((center->x - center->x)*(center->x - collider->center->x) + (center->y - collider->center->y)*(center->y - collider->center->y));
		//normal
		float nx = (collider->center->x - center->x) / Distance;
